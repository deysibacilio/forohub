...
class ForhubApplicationTests {
	...
	void testTopicoRepository() {
		...
		when(topicoRepository.findByTitulo("Título de prueba")).thenReturn(mockTopico);
	...
	void testUsuarioRepository() {
		...
		when(usuarioRepository.findByLogin("usuarioDePrueba")).thenReturn(mockUsuario);
